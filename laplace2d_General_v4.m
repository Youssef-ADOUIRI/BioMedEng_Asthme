function [A,B]=laplace2d_General_v4(F,nx,ny,L,D , eta , abs1 , ord1 , long1 , abs2 , ord2 , long2)
...
% conditions a la derive normale hors 3
% for i=2:abs3-1
%     k=i;
%     Y3(k,:) = 0; A3(k,:) = 0; P3(k,:) = 0;
%     % Y3(k,k) = 1.0; Y3(k ,k + nx) = -1.0;
...
%     P3(k,k)=beta/eta;P3(k,k+2*nx)=-beta/eta;
% end
for i=abs3+long3+1:nx-1
    k=i;
    Y3(k,:) = 0; A3(k,:) = 0; P3(k,:) = 0;
    Y3(k,k+[ -nx, -1, 0, 1, nx]+nx)=coeff;
    P3(k,k)=beta/eta;P3(k,k+2*nx)=-beta/eta;
end
%derivee normale dans obstacle 3
% for i=abs3+1:abs3+long3-1
%     k=(ord2 - 1)*nx +i;
%     Y3(k,:) = 0; A3(k,:) = 0; P3(k,:) = 0;
%     Y3(k,k+[ -nx, -1, 0, 1, nx]+nx)=coeff;
%     P3(k,k)=beta/eta;P3(k,k+2*nx)=-beta/eta;
...
%derivee normale gauche droite obstocale 3
% for j=2:ord3-1
%     k=(j-1)*nx+abs3; 
%     A3(k,k) = 1.0; A3(k ,k - 1) = -1.0;
% 
...
%mettre des valeurs nulles a l'interieur d'obstacle 3
% for i=abs3:abs3+long3
%     for j=1:ord3
%         k=(j-1)*nx+i;
%         A1(k,:) =0;
%         P1(k,:) =0;
...
%pression nulle a l'interieur d'obstacle 3
% for i=abs3+1:abs3+long3-1
%     for j=1:ord3-1
%         k=(j-1)*nx + i;
%         A3(k,:) = 0;
...
%conditions corners immerges 3
% for k=[(ord3 - 1)*nx + abs3,  (ord3-1)*nx+abs3 + long3]
%     Y3(k,k) = 1.0; Y3(k , k + nx) = -1.0;
% end
...
%conditions corners parois 3
% for k=[abs3,  abs3 + long3]
%     
%     Y3(k,k+1) =a;
%     Y3(k,k-1)=a;
%     Y3(k,k)=c;
